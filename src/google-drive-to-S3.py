...
def main_handler(event, context):
    ...
    os.chdir('/tmp/')
    request = service.files().get_media(fileId='1SrhmjVCW6dDV_vStCZHRzwI5dO_snvAo')
    fh = io.FileIO('testfile.xlsx', 'wb')
    downloader = MediaIoBaseDownload(fh, request)
    done = False
    while done is False:
        status, done = downloader.next_chunk()
    ...
    s3_client.upload_file("/tmp/testfile.xlsx", "gdil-s3", "testfile.xlsx")